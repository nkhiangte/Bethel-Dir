  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors">{resident.name}</h3>
          <p className="text-sm text-gray-500 font-medium">House No: {resident.houseNumber}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button 
          ...
          className="flex items-center justify-center gap-2 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition-colors shadow-sm active:scale-95"
        >
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-bold hover:bg-emerald-600 transition-colors shadow-sm active:scale-95"
        >
      ...
      {isAdmin && (
        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            ...
            className="p-2 bg-white/80 backdrop-blur-sm border border-gray-100 rounded-lg text-gray-600 hover:text-indigo-600 hover:border-indigo-100 transition-all shadow-sm"
          >
            <Edit2 className="w-4 h-4" />
          </button>
          <button 
            onClick={() => resident.id && onDelete(resident.id)}
            className="p-2 bg-white/80 backdrop-blur-sm border border-gray-100 rounded-lg text-gray-600 hover:text-red-600 hover:border-red-100 transition-all shadow-sm"
          >
        ...
        <div className="px-8 pt-8 pb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 ml-1">House No</label>
                <input 
                  required
            ...
            <button 
              type="submit"
              className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold text-lg hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 active:scale-95 flex items-center justify-center gap-2 mt-4"
            >
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-40 backdrop-blur-md bg-white/80">
        <div className="max-w-4xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-900 tracking-tight leading-none mb-1">Bethel Veng</h1>
              <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Champhai Directory</p>
            </div>
          </div>
            ...
            {user ? (
              <>
                <div className="hidden sm:block text-right">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{userProfile?.role}</p>
                </div>
                <button 
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-bold text-sm hover:bg-indigo-100 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoggingIn ? (
      ...
      <main className="max-w-4xl mx-auto px-4 pt-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 bg-white border border-gray-100 rounded-2xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all font-medium"
            />
          </div>
          {isAdmin && (
            <div className="flex flex-wrap gap-2">
              <button 
                ...
                className="px-4 py-4 bg-white border border-gray-100 text-gray-400 hover:text-indigo-600 rounded-2xl font-bold hover:bg-indigo-50 transition-all shadow-sm flex items-center justify-center gap-2 active:scale-95 shrink-0"
                title="Download Import Template"
              >
                <Download className="w-5 h-5" />
                Template
              </button>
              <label className="px-6 py-4 bg-white border border-gray-100 text-gray-600 rounded-2xl font-bold hover:bg-gray-50 transition-all shadow-sm flex items-center justify-center gap-2 active:scale-95 shrink-0 cursor-pointer">
              ...
              <button 
                ...
                className="px-6 py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 active:scale-95 shrink-0"
              >
        ...
        <div className="flex gap-4 mb-8 overflow-x-auto pb-2 scrollbar-hide">
          <div className="bg-white px-6 py-4 rounded-2xl border border-gray-100 shadow-sm min-w-[140px]">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Total Residents</p>
            <p className="text-2xl font-black text-gray-900">{residents.length}</p>
          </div>
          <div className="bg-white px-6 py-4 rounded-2xl border border-gray-100 shadow-sm min-w-[140px]">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Results Found</p>
            <p className="text-2xl font-black text-indigo-600">{filteredResidents.length}</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-gray-200">
            ...
            <p className="text-sm text-gray-400 font-medium">Try searching with a different term</p>
          </div>
        )}
      </main>
